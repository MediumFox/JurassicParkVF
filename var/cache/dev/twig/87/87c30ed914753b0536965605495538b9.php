...
class __TwigTemplate_2fb823d9d069db09f3aaf0223f650b16 extends Template
{
...
<script>
    ...
    function renderCards(centerIndex = 0) {
        ...
        visible.forEach((day, index) => {
            ...
            html += `
                <div class=\"bg-white shadow-xl rounded-xl p-6 cursor-pointer \${index === 2 ? 'scale-110 opacity-100 z-10 mx-2' : 'opacity-75'}
                            text-center w-[18%] transition-all duration-300\"
                     onclick=\"renderCards(\${(centerIndex + index) % forecastData.length})\">
                    <p class=\"font-semibold text-slate-900 mb-2\">\${dateFr}</p>
                    <img src=\"https:\${icon}\" alt=\"\${condition}\" class=\"mx-auto mb-2 w-16 h-16\">
                    <p class=\"text-bleuLune font-bold text-lg mb-1\">\${conditionFr}</p>
                    <p class=\"text-bleuGlace italic text-sm\">\${min}°C / \${max}°C</p>
                </div>
            `;
        });
